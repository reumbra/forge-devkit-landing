...
| Свойство | Тип | Описание | Примеры значений |
|---|---|---|---|
| `section_id` | string | HTML id секции | `hero`, `problems`, `solution`, `pipeline`, `deep-architecture`, `deep-quality`, `deep-pipeline`, `how-it-works`, `modules`, `dashboard`, `pricing`, `author`, `faq`, `final-cta` |
...
### 2.4 Pricing-события (GA4 recommended + кастомные)
...
### 2.5 Language-события (кастомные)
...
### 2.7 Comparison-события (кастомные)
...
**Итого: 3 автоматических + 4 GA4 recommended + 9 кастомных = 16 событий.**
